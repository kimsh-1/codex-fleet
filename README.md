...
`codex exec`(비대화형 Codex CLI)를 백그라운드로 N개 동시에 띄워서, 이미지든 임의 작업이든 나눠 대량으로 뽑는다. 처리량은 스폰 개수 하나(`PARALLEL`)로 조절하고, 결과는 파일로 회수한다. **Codex CLI를 그대로 여러 개 띄우는 CLI 양산 시스템**이다.
